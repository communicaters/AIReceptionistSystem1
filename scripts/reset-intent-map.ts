 ...
 * 
 * Usage: npx tsx scripts/reset-intent-map.ts
 */

import { pool, db } from '../server/db';
import { intentMap } from '../migrations/schema';
...
async function resetIntentMap() {
  ...
  try {
    ...
    const count = await db.select({ count: { count: intentMap.id } }).from(intentMap);
    console.log(`Verification: There are now ${count[0].count.count} intent records in the system`);
